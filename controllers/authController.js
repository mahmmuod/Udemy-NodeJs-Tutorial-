...
exports.restrictTo = (...roles) => {
  return (req, res, next) => {
    ...
    if (!roles.includes(req.user.role)) {
      //req.user.role will give you the current user from the middleware before
      return next(
...
exports.forgetPassword = catchAsync(async (req, res, next) => {
  ...
  const message = `Forget your Password? submit a patch request with your new password and passwordConfirm to: ${resetURL}/n
  ...
  try {
    ...
    return next(
      new AppError(
        `There was an error sending the email. Try agin later ${err}`
      ),
      500
    );
  }
